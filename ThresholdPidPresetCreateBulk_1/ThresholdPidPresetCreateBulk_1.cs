...
namespace ThresholdPidPresetCreateBulk
{
	using System;
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			command.Source = new Source("BT VB Series - InterAppDemo - Threshold - PID Preset - Create Bulk");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			for (int i = 1; i <= 10; i++)
			{
				var message = new CreatePidThresholdPreset
				{
					ThresholdName = thresholdName,
					ThresholdPresetData = MakeThresholdPresetData(i),
					Source = new Source("BT VB Series - InterAppDemo - Thresholds - PID Preset - Create Bulk"),
				};
			...
			foreach (var responseMessage in command.Send(Engine.SLNetRaw, element.DmaId, element.ElementId, 9_000_000, new TimeSpan(0, 0, 10), InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is CreatePidThresholdPresetResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(CreatePidThresholdPresetResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
		...
		public PidThresholdPresetData MakeThresholdPresetData(int i)
		{
			var presetData = new PidThresholdPresetData
			{
				Type = PresetType.PID,
				Pid = $"1{i}",
				PidDescription = $"Demo_CreateBulk_1{i}",
