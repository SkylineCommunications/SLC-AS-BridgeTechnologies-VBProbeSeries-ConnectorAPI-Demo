...
namespace ThresholdEthEdit
{
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			command.Source = new Source("BT VB Series - InterAppDemo - Thresholds - Eth - Edit");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new EditEthThreshold
			{
				ThresholdToUpdateName = thresholdName,
				ThresholdData = MakeThresholdData(),
				Source = new Source("BT VB Series - InterAppDemo - Thresholds - Eth - Edit"),
			};
			...
			foreach (var responseMessage in command.Send(Engine.SLNetRaw, element.DmaId, element.ElementId, 9000000, new TimeSpan(0, 0, 10), InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is EditEthThresholdResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(EditEthThresholdResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
		...
		{
			var thresholdData = new EthThresholdData
			{
