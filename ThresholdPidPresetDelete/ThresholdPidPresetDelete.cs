...
namespace ThresholdPidPresetDelete
{
	using System;
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			command.Source = new Source("BT VB Series - Thresholds - PID Preset - Delete");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new DeletePidThresholdPreset
			{
				ThresholdPresetKey = presetKey,
				Source = new Source("BT VB Series - Thresholds - PID Preset - Delete"),
			};
				...
				InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is DeletePidThresholdPresetResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(DeletePidThresholdPresetResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
