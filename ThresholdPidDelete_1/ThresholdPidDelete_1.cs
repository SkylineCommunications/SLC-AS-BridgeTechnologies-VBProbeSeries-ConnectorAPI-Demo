...
namespace ThresholdPidDelete
{
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			command.Source = new Source("BT VB Series - Thresholds - PID - Delete");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new DeletePidThresholds
			{
				ThresholdToDeleteNames = thresholdNames.Split(';'),
				////DeleteAllOrNone = true,
				Source = new Source("BT VB Series - Thresholds - PID - Delete"),
			};
				...
				InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is DeletePidThresholdsResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(DeletePidThresholdsResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
