...
namespace ThresholdEtrDelete
{
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			string thresholdNames = engine.GetScriptParam("Threshold Names").Value;
			...
			command.Source = new Source("BT VB Series - Thresholds - ETR - Delete");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new DeleteEtrThresholds
			{
				ThresholdToDeleteNames = thresholdNames.Split(';'),
				////DeleteAllOrNone = true,
				Source = new Source("BT VB Series - Thresholds - ETR - Delete"),
			};
				...
				InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is DeleteEtrThresholdsResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(DeleteEtrThresholdsResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
