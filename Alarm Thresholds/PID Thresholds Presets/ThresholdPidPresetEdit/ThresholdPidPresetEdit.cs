...
namespace ThresholdPidPresetEdit
{
	using System;
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			string presetKey = engine.GetScriptParam("Threshold Preset Key").Value;
			...
			command.Source = new Source("BT VB Series - InterAppDemo - Thresholds - PID Preset - Edit");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new EditPidThresholdPreset
			{
				ThresholdPresetKey = presetKey,
				ThresholdPresetNewData = MakeThresholdData(),
				Source = new Source("BT VB Series - InterAppDemo - Thresholds - PID Preset - Edit"),
			};
				...
				InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is EditPidThresholdPresetResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(EditPidThresholdPresetResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
		...
		public PidThresholdPresetData MakeThresholdData()
		{
			var thresholdData = new PidThresholdPresetData
			{
				Type = PresetType.PID,
				Pid = "0",
				PidDescription = $"Demo_EditSingle_0",

				RequirePresence = null,
				RequiredLanguage = null,
				MonitorScrambling = null,

				MonitorMinBitRate = null,
				MinBitRateValue = null,

				MonitorMaxBitRate = null,
				MaxBitRateValue = null,

				IgnorePcrErrorsSchedule = null,
				IgnoreCcErrors = null,
				IgnoreMissingPidAlarm = null,
				IgnoreUnreferencedPidAlarm = null,
				IgnoreAll = null,
			};
