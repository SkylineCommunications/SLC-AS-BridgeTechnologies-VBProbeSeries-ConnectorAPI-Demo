...
namespace ThresholdEthDelete
{
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			command.Source = new Source("BT VB Series - Thresholds - Eth - Delete");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new DeleteEthThresholds
			{
				ThresholdToDeleteNames = thresholdNames.Split(';'),
				////DeleteAllOrNone = true,
				Source = new Source("BT VB Series - Thresholds - Eth - Delete"),
			};
				...
				InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is DeleteEthThresholdsResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(DeleteEthThresholdsResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
