...
namespace ThresholdEtrEdit
{
	...
	public class Script
	{
		...
		public void Run(IEngine engine)
		{
			...
			command.Source = new Source("BT VB Series - InterAppDemo - Thresholds - ETR - Edit");
			command.ReturnAddress = new ReturnAddress(element.DmaId, element.ElementId, 9000001);

			var message = new EditEtrThreshold
			{
				ThresholdToUpdateName = thresholdName,
				ThresholdData = MakeThresholdData(),
				Source = new Source("BT VB Series - InterAppDemo - Thresholds - ETR - Edit"),
			};
			...
			foreach (var responseMessage in command.Send(Engine.SLNetRaw, element.DmaId, element.ElementId, 9000000, new TimeSpan(0, 0, 10), InterApp.KnownTypes))
			{
				...
				{
					if (responseMessage is EditEtrThresholdResult result)
					{
						engine.GenerateInformation(result.Description);
					}
					else
					{
						engine.GenerateInformation($"{nameof(responseMessage)} is not of expected type '{nameof(EditEtrThresholdResult)}'.{Environment.NewLine}{responseMessage}");
					}
				}
				else
				{
